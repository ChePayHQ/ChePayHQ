This directory contains the ChePay Qt graphical user interface (GUI). It uses
the cross-platform framework [Qt](https://www.qt.io/).
...
Represents the main window of the ChePay UI.
...
ClientModel is used by the main application and several models like
`peertablemodel`.
...
Used to process BIP21 payment URI requests. Also handles URI-based application
switching when following a `chepay:...` link from a browser.
...
* `bitcoinunits.(h/cpp)`: CPY / mCPY / etc handling
* `callback.h`
...
10. Start debugging with Qt Creator (you might need to set the executable to
    `litecoin-qt` under "Run", which is where you can also add command-line
    arguments)
