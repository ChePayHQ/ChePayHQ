...
- [Development guidelines](#development-guidelines)
    - [General ChePay Core](#general-litecoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
ChePay Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
ChePay Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/ChePayHQ/ChePayHQ/blob/master/contrib/valgrind.supp))
...
ChePay Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as ChePay Core, where everyone uses
...
pay attention to for reviewers of ChePay Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: ChePay Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of ChePay Core, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against ChePay Core so that they can be integrated
...
file descriptors, it will cause problems with ChePay's `select()` loop, because
...
compatibility between nodes. This happened in ChePay 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to ChePay's
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of ChePay Core, the wallet was always
