ChePay Core version 0.18.1 is now available from:
...
`/Applications/ChePay-Qt` (on Mac) or `litecoind`/`litecoin-qt` (on
Linux).
...
ChePay Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use ChePay Core on unsupported systems.
...
10.10. Additionally, ChePay Core does not yet change appearance when
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@litecoin.org) the ChePay Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 CPAY),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in CPAY per kilobyte, not litoshi per byte.
...
- A new [document](https://github.com/litecoin-project/litecoin/blob/master/doc/litecoin-conf.md)
  about the `chepay.conf` file describes how to use it to configure
  ChePay Core.

- A new document introduces ChePay Core's BIP174 [Partially-Signed
  ChePay Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent ChePay-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  ChePay Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when ChePay Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of ChePay Core, subsequently importing
...
- A new `litecoin-wallet` tool is now distributed alongside ChePay
...
This section describes planned changes to ChePay Core that may affect
other ChePay software and services.

- Since version 0.16.0, ChePay Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with ChePay Core 0.20 (expected about
  a year after 0.18), ChePay Core will default to native segwit
  ...
  to bech32 addresses, and if the ChePay Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in ChePay Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any ChePay Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, ChePay Core
...
- A sub-project of ChePay Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with ChePay Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to ChePay Core's own implementation, although entropy
  gathered by ChePay Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves ChePay Core a
...
- On macOS, ChePay Core now opts out of application CPU throttling
