ChePay Core version 0.13.2 is now available from:
...
ECDSA signatures inside ChePay transactions now use validation using
...
Previous versions of ChePay Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of ChePay Core was to raise the default minimum
relay fee.

ChePay Core 0.13.2 will have a strict maximum size on the mempool. The
...
ChePay Core 0.13.2 also introduces new default policy limits on the length and
...
ChePay Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. ChePay Core will relay transactions with
...
entirely in ChePay Core 0.13, and it is at this time undecided whether the
...
ChePay Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
ChePay Core automatically creates a hidden service to listen on, without
manual configuration. ChePay Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if ChePay Core is listening, and
...
ChePayd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals ChePay
Core to use floating fees. By default, ChePay Core will use floating
fees.
...
(default: `0.0002` CPAY/kB).

At all times, ChePay Core will cap fees at `-maxtxfee=<x>` (default:
0.10) CPAY.
Furthermore, ChePay Core will never create transactions paying less than
...
`chepay.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
- Adding `dbcache=100` in chepay.conf
...
Various code modernizations have been done. The ChePay Core code base has
...
for a new opcode in the ChePay scripting system that in combination with
...
ChePay transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of ChePay Core.
...
The two phases are distinct in the ChePay-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming ChePay wallet
  software more difficult and which seriously complicates the design of smart
  contracts for ChePay.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each ChePay block need to be stored by nodes in order to validate future
...
- **Increased security for multisig:** ChePay addresses (both P2PKH addresses
  ...
  believed to be provided by ChePay's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of ChePay's current ledger (called the UTXO set),
  ...
  note that ChePay Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  ChePay users to individually opt-in to almost any change in the ChePay
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and ChePay Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the ChePay
client to ChePay Core 0.13.2 to signal for segwit support. When a
...
Since ChePay Core 0.10.0, nodes have defaulted to only relaying and
