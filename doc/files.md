...
The data directory is the default location where the ChePay Core files are stored.
...
---------|--------------------
Linux    | `$HOME/.litecoin/`
macOS    | `$HOME/Library/Application Support/ChePay/`
Windows  | `%APPDATA%\ChePay\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for `chepay.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual ChePay blocks (in network format, dumped in raw on disk, 128 MiB per file)
...
`./`               | `chepay.conf`        | User-defined [configuration settings](bitcoin-conf.md) for `litecoind` or `litecoin-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
...
These subdirectories and files are no longer used by the ChePay Core:
